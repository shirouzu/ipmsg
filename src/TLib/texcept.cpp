...
BOOL RegisterDumpExceptArea(void *ptr, size_t size)
{
	if (DumpExceptAreaIdx >= MAX_DEA_NUM || !ptr || !size) {
		Debug("RegisterDumpExceptArea err(%d %p %zx)\n", DumpExceptAreaIdx, ptr, size);
		return	FALSE;
	}
	...
	Debug("RegisterDumpExceptArea (%d %p %zx)\n", DumpExceptAreaIdx, ptr, size);
...
{
	if (!ptr || !size) {
		Debug("ModifyDumpExceptArea is null(%p %zx)\n", ptr, size);
	...
	for (int i=0; i < DumpExceptAreaIdx; i++) {
		...
		if (area.ptr == ptr) {
			area.size = size;
			Debug("ModifyDumpExceptArea (%d %p %zx)\n", i, ptr, size);
			return	TRUE;
		}
	}
	Debug("ModifyDumpExceptArea not found (%p %zx)\n", ptr, size);
	return	FALSE;
}
...
{
	if (!ptr) {
		Debug("RemoveDumpExceptArea is null\n");
		return	FALSE;
	}
	for (int i=0; i < DumpExceptAreaIdx; i++) {
		auto	&area = DumpExceptArea[i];
		if (area.ptr == ptr) {
			Debug("RemoveDumpExceptArea (%d %p %zx)\n", i, ptr, area.size);
			memmove(&area, &area + 1, sizeof(area) * (DumpExceptAreaIdx - i -1));
			DumpExceptAreaIdx--;
			return	TRUE;
		}
	}
	Debug("RemoveDumpExceptArea not found (%p)\n", ptr);
	return	FALSE;
}

void InitDumpExceptArea()
{
	Debug("InitDumpExceptArea\n");
	DumpExceptAreaIdx = 0;
}
